...
function section(n,title,limit,items,compact=false){return `<section><div class="section-title"><h2>${n}. ${title}</h2><span>${limit}</span></div><div class="grid">${items.length?items.map(x=>card(x,compact)).join(''):'<article class="story-card compact"><p>No strong signal right now. TBAR stays quiet rather than filling space.</p></article>'}</div></section>`;}
function render(brief,{live=false,updatedAt=null}={}){document.querySelector('#root').innerHTML=`<main class="app-shell"><header class="topbar"><div><div class="brand">TBAR</div><div class="subtitle">Taiwan Business & AI Radar</div></div><div class="live-dot"><i></i>${live?'LIVE':'DEMO'}</div></header><section class="hero"><span class="eyebrow">${live?'LIVE · '+fmtTime(updatedAt):'TODAY'}</span><h1>What matters. Nothing more.</h1><p>AI + business intelligence filtered through a Taiwan lens.</p></section>${section(1,'Must Know','≤ 3',brief.mustKnow)}${section(2,'Important Signals','≤ 5',brief.importantSignals,true)}${section(3,'Deep Dive','0–2',brief.deepDive)}<div class="country-row">${COUNTRY_DEEP_DIVE.map(c=>`<span>${c}</span>`).join('')}</div>${section(4,'Watchlist','living signals',brief.watchlist,true)}<section class="leaders"><div class="section-title"><h2>AI Leader Radar</h2><span>10 leaders</span></div><div class="chips">${AI_LEADERS.map(p=>`<span>${p}</span>`).join('')}</div></section><footer>TBAR V0.3 · ${live?'Live candidates + judgment':'Demo fallback'} · Human review remains the publishing boundary</footer></main>`;}
