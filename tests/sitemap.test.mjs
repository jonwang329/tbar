...
test('parseSitemap extracts loc and lastmod', () => {
  ...
  assert.equal(parsed.urls[0].url, 'https://example.com/news/alpha');
  assert.equal(parsed.urls[0].lastmod, '2026-08-13T01:02:03Z');
});

test('filterSitemapEntries respects include path', () => {
  const entries = filterSitemapEntries(parseSitemap(xml).urls, { includePath: '/news/' });
...
test('parseSitemap detects sitemap indexes', () => {
  const parsed = parseSitemap('<sitemapindex><sitemap><loc>https://example.com/posts.xml</loc></sitemap></sitemapindex>');
  assert.deepEqual(parsed.sitemaps, ['https://example.com/posts.xml']);
});
